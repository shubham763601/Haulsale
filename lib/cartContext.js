...
export function CartProvider({ children }) {
  ...
  const addItem = (variant, qty = 1) => {
    setItems(prev => {
      const idx = prev.findIndex(i => String(i.variant_id) === String(variant.variant_id ?? variant.id))
      if (idx !== -1) {
        const copy = [...prev]
      ...
      return [
        ...prev,
        {
          ...
          title: variant.product_title ?? variant.product_title ?? variant.product_title ?? variant.title ?? '',
