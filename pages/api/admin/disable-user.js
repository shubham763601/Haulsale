...
export default async function handler(req, res) {
  ...
  if (!user_id || typeof disable === 'undefined') return res.status(400).json({ error: 'user_id and disable required' })

  try {
    const { error } = await supabaseAdmin
      .from('profiles')
      .update({ disabled: disable })  -- /* if you don't have disabled column, change below to create or use 'is_disabled' */
      .eq('id', user_id)
